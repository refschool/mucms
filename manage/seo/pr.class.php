...
public $google_host ='toolbarqueries.google.com';
public $google_ua ='Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.0.6) Gecko/20060728 Firefox/1.5';
...
function getpr($url) {
	...
	$ch = $this->getch($url);
	$fp = fsockopen($this->google_host, 80, $errno, $errstr, 30);
	if ($fp) {
	   //$out = "GET /search?client=navclient-auto&ch=$ch&features=Rank&q=info:$url HTTP/1.1\r\n";
	   $out = "GET /tbr?client=navclient-auto&ch=$ch&features=Rank&q=info:$url HTTP/1.1\r\n";//new url
	   //echo "<pre>$out</pre>\n"; //debug only
	   $out .= "User-Agent: ".$this->google_ua."\r\n";
	   $out .= "Host: ".$this->google_host."\r\n";
...
function pagerank($url,$width=40,$method='style') {
	...
	//The pre-styled method preferred
	if ($method == 'style') {
	$prpercent=100*$pr/10;
...
//custom functions for mucms
//
function showpr($url){
	$pr = pagerank($url);echo $pr;
	//return $pr;
}
...
/**
usage :
$prc = new pagerankChecker();
echo $prc->getpr('www.china-roots.com');
